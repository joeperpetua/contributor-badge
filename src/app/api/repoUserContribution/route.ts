...
export async function GET(request: NextRequest) {
  try {
    ...
    const themeOptions = {
      ...
      transparent:
        searchParams.get("transparent")?.toLowerCase() === "true"
          ? true
          : false, // default false
      showOwner:
        searchParams.get("showOwner")?.toLowerCase() === "false" ? false : true, // default true
    ...
    if (!owner || !repo || !user) {
      ...
      return NextResponse.json(
        {
          error: "Bad Request",
          message: "Missing owner, repo or user parameter.",
        },
        { status: 400 },
      );
    }
    ...
    const multiline =
      owner.length + repo.length > 22 && themeOptions.showOwner ? true : false;
    ...
    console.error(error);
    return NextResponse.json(
      { error: "Error while generating badge", message: `${error}` },
      { status: 500 },
    );
  }
}
